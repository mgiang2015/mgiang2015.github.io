...
export function UploadDocumentsForm() {
  ...
  const ingest = async (e) => {
    ...
    const response = await fetch(`http://54.147.175.219/rag-pinecone/website?url=${document}`, {
      method: "POST",
    });
  ...
  return (
    ...
    <textarea
      className="grow mr-8 p-4 rounded border-solid border-2 border-slate-800"
      value={document}
      onChange={(e) => setDocument(e.target.value)}
    ></textarea>
    <button type="submit" className="shrink-0 bg-sky-600 rounded w-28">
      ...
      <span className={isLoading ? "hidden" : ""}>WEAVE!</span>
    </button>
  </form>
  );
}